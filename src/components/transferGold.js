...
export async function transfer(wallet) {
  if (
    !process.env.NEXT_PUBLIC_FROM_NFT ||
    !process.env.NEXT_PUBLIC_TO_NFT ||
  ...
  const fromNft = await metaplex.nfts().findByMint({
    mintAddress: new PublicKey(process.env.NEXT_PUBLIC_FROM_NFT),
  });

  const toNft = await metaplex.nfts().findByMint({
    mintAddress: new PublicKey(process.env.NEXT_PUBLIC_TO_NFT),
  });
  ...
  console.log("Transfering rewards...")
  const amount = new BN(process.env.NEXT_PUBLIC_TRANSFER_AMOUNT); // however much u want here
